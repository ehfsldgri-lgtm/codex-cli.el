...
(ert-deftest codex-cli-test--resume-session-starts-codex-resume ()
  "Resuming should start a new buffer with `codex resume'."
  (let ((session-buffer (generate-new-buffer " *codex-session*"))
        started
        shown)
    (unwind-protect
        (cl-letf (((symbol-function 'codex-cli-project-root)
                   (lambda () "/tmp/project/"))
                  ((symbol-function 'codex-cli--sessions-for-project)
                   (lambda () nil))
                  ((symbol-function 'codex-cli--generate-session-id)
                   (lambda () "dev"))
                  ((symbol-function 'codex-cli--get-or-create-buffer)
                   (lambda (_name) session-buffer))
                  ((symbol-function 'codex-cli--start-terminal-process)
                   (lambda (buffer root command args backend)
                     (setq started (list buffer root command args backend))))
                  ...
                  ((symbol-function 'codex-cli--show-and-maybe-focus)
                   (lambda (buffer)
                     (setq shown buffer))))
          (let ((codex-cli-executable "codex")
                (codex-cli-extra-args '("--search"))
...
(ert-deftest codex-cli-test--resume-session-rejects-duplicate-name ()
  "Resuming with an explicit duplicate session name should error."
  (cl-letf (((symbol-function 'codex-cli-project-root)
             (lambda () "/tmp/project/"))
            ((symbol-function 'codex-cli--sessions-for-project)
             (lambda () '("dev"))))
    (should-error (codex-cli-resume-session "dev") :type 'user-error)))

(ert-deftest codex-cli-test--toggle-in-codex-buffer-uses-global-chooser ()
  "Toggling from a Codex buffer should choose among all live sessions."
  (let ((current (generate-new-buffer "*codex-cli:proj-a:dev*"))
        (other (generate-new-buffer "*codex-cli:proj-b:work*"))
        set-target)
    (unwind-protect
        (with-current-buffer current
          (cl-letf (((symbol-function 'codex-cli--project-session-buffers)
                     (lambda () (list current)))
                    ((symbol-function 'codex-cli--all-session-buffers)
                     (lambda () (list current other)))
                    ((symbol-function 'get-buffer-window)
                     (lambda (buffer &optional _all-frames)
                       (when (eq buffer current) 'win)))
                    ((symbol-function 'window-buffer)
                     (lambda (_window) current))
                    ((symbol-function 'codex-cli--choose-any-session-buffer)
                     (lambda (&optional _prompt) other))
                    ((symbol-function 'codex-cli--choose-project-session-buffer)
                     (lambda (&optional _prompt)
                       (ert-fail "Project chooser should not be used here")))
                    ((symbol-function 'codex-cli--record-last-session)
                     (lambda (&rest _)))
                    ((symbol-function 'codex-cli--session-name-for-buffer)
                     (lambda (_buffer) "work"))
                    ((symbol-function 'set-window-buffer)
                     (lambda (_window buffer)
                       (setq set-target buffer)))
                    ((symbol-function 'delete-window)
                     (lambda (&rest _)
                       (ert-fail "Current window should switch, not close"))))
            (codex-cli-toggle)
            (should (eq set-target other))))
      (dolist (buf (list current other))
